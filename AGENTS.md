...
### MEMORY & PLANNING SKILLS (Codex-mem) — use for search, planning, execution

| Skill | Trigger | When |
|---|---|---|
| `Codex-mem:make-plan` | "plan", "design the system", "how do we build" | Phase 0 docs discovery first |
| `Codex-mem:do` | "execute", "run the plan", "implement" | Subagent-driven with verification |
| `Codex-mem:mem-search` | "did we solve this", "how did we do X", past work | 3-layer search, ~10x token savings |
| `Codex-mem:smart-explore` | Exploring code structure, "where is X defined" | AST parsing, 4-8x token savings |
| `Codex-mem:learn-codebase` | "prime", "learn the codebase", new project start | Read every file systematically |
| `Codex-mem:timeline-report` | "project history", "what have we built" | Full narrative of development |
| `Codex-mem:knowledge-agent` | "build a brain for X", "what do we know about Y" | Filter corpus from observations |
| `Codex-mem:pathfinder` | "map the architecture", "find duplicated logic" | Mermaid flowcharts + refactor plan |
| `Codex-mem:babysit` | "watch this PR", "wait for CI" | Poll until merge-ready |
| `Codex-mem:version-bump` | "release", "bump version", "publish" | Full release workflow |
| `Codex-mem:timeline-report` | "project journey", "history" | 3000-6000 word narrative |
| `Codex-mem:how-it-works` | "how does Codex-mem work" | Architecture explanation |
...
```bash
python3 ~/.Codex/skills/ui-ux-pro-max-skill/src/ui-ux-pro-max/scripts/search.py "<query>" --domain <style|color|typography|ux|landing|chart|product>
# Stack options: html-tailwind, react, nextjs, astro, vue, nuxtjs, shadcn, react-native, flutter
python3 ~/.Codex/skills/ui-ux-pro-max-skill/src/ui-ux-pro-max/scripts/search.py "<query>" --stack nextjs
```
...
- **Memory:** Codex-mem captures all sessions automatically

---

## AWESOME-Codex REFERENCE

Browse 200+ curated Codex resources: `github.com/hesreallyhim/awesome-Codex`
...
- **Orchestrators:** Codex Task Master, Codex Flow, Codex Swarm
